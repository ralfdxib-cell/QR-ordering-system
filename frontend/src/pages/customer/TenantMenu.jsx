...
import { categoryAPI, menuItemAPI, tableAPI } from '../../lib/api';
...
export default function TenantMenu() {
  ...
  const { table, setCurrentTable, setCurrentTenant, cart, addToCart, getCartCount, getCartTotal } = useCart();
  ...
  const initializeRestaurant = async () => {
    try {
      ...
      const settingsData = await fetchPublicSettings(slug);
      setCurrentTenant({
        id: settingsData.id,
        slug: slug,
      ...
      const [catResponse, itemsResponse] = await Promise.all([
        categoryAPI.getPublic(slug),
        menuItemAPI.getPublic(slug)
      ]);
      
      setCategories(catResponse.data);
      setMenuItems(itemsResponse.data);
      
      // For demo mode, auto-select first table
      const isDemo = searchParams.get('demo') === 'true';
      if (isDemo && !table) {
        const tablesResponse = await tableAPI.getPublic(slug);
        if (tablesResponse.data.length > 0) {
          setCurrentTable(tablesResponse.data[0]);
        }
      }
    } catch (error) {
      console.error('Error initializing restaurant:', error);
      setError('Restaurant niet gevonden');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchMenuItems = async (categoryId = null) => {
    try {
      const response = await menuItemAPI.getPublic(slug, categoryId);
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.dietary_tags.map((tag) => {
                        ...
                        return (
                          <span
                            key={tag}
                            className={`dietary-badge ${tagConfig?.color || 'bg-gray-100 text-gray-700'}`}
                          >
                            {dietaryTagLabelsNL[tag] || tag}
                          </span>
                        );
                      })}
                    </div>
              ...
              <DrawerHeader className="relative p-0">
                ...
                {selectedItem.image_url && (
                  <div className="h-48 w-full">
                    <img
                      src={selectedItem.image_url.startsWith('/api') ? `${process.env.REACT_APP_BACKEND_URL}${selectedItem.image_url}` : selectedItem.image_url}
